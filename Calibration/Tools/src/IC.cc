...
void IC::PrintInfos(void)
{
	for (size_t i = 0; i < ids().size(); ++i) {
		DetId id(ids()[i]);
		Coord c;
		coord(id, &c);
		std::cout << c.ix_ << "\t" << c.iy_ << "\t" << c.iz_ << "\t" << id.rawId() << std::endl;
	}
}
...
void IC::operator /=(float val)
{
	for (size_t i = 0; i < ids().size(); ++i) {
		...
		ic().setValue(id, va / val);
		eic().setValue(id, ea / val);
	}
	return;
}
...
void IC::readXMLFile(const char * fileName)
{
	...
	while(file.peek() != EOF && file.good()) {
		...
		if(start_pos != std::string::npos) {
			...
			end_pos = line_string.find("</item>");
			assert(end_pos != std::string::npos);

			std::string val = line_string.substr(start_pos, end_pos - start_pos); //15);
...
float IC::Normalize(DS& selector)
{
	...
	sum /= n; //calculate the average
	std::cout << "Average is: " << sum << std::endl;
	//divide by the average
	for (size_t i = 0; i < ids().size(); ++i) {
		DetId id(ids()[i]);
		if (!selector(id)) continue;
		ic().setValue(id, ic()[id] / sum);
		eic().setValue(id, eic()[id] / sum);
	}
	return sum;
...
void IC::BonToBoff(const IC& Bcorr, const IC& alphas, DS& selector)
{
	std::cout << "[WARNING] No corrections will be applied to EB!" << std::endl;

	for (size_t i = 0; i < ids().size(); ++i) {
		DetId id(ids()[i]);
		if(!selector(id)) continue;
		float newIC = ic()[id] * std::pow(Bcorr.ic()[id], alphas.ic()[id]);
		ic().setValue(id, newIC);
	}
