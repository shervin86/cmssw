                                ...
                                const edm::EventSetup& iSetup)
{
	...
	const EBUncalibratedRecHitCollection *barrelUncalibHitsCollection = barrelUncalibRecHitsHandle.isValid() ? barrelUncalibRecHitsHandle.product () : nullptr;
	...
	const EEUncalibratedRecHitCollection *endcapUncalibHitsCollection = endcapUncalibRecHitsHandle.isValid() ?  endcapUncalibRecHitsHandle.product () : nullptr;
	...
	auto miniEBUncalibRecHitCollection = std::make_unique<EBUncalibratedRecHitCollection>();
	auto miniEEUncalibRecHitCollection = std::make_unique<EEUncalibratedRecHitCollection>();
	...
	for(std::set<DetId>::const_iterator itr = reducedRecHit_EBmap.begin();
		...
		if (barrelUncalibHitsCollection != nullptr && barrelUncalibHitsCollection->find(*itr) != barrelUncalibHitsCollection->end())
	...
	for(std::set<DetId>::const_iterator itr = reducedRecHit_EEmap.begin();
		...
		if (endcapUncalibHitsCollection != nullptr && endcapUncalibHitsCollection->find(*itr) != endcapUncalibHitsCollection->end())
